...
def cutting_cloth(X, Y, a, b, p):
	C = np.zeros((X + 1,Y + 1))
	S = np.zeros((X + 1,Y + 1), dtype=np.dtype('U100000')) #WARNING - LIMIT STRING
	...
	solution = []
	text_solution = S[X][Y]
	for i in range(0, len(text_solution)):
		if text_solution[i] != '/': solution.append(text_solution[i])
...
#print(cutting_cloth(40,70, [21,31,9,9,30,11,10,14,12,13],
#	                       [22,13,35,24,7,13,14,8,8,7], 
#                           [582,403,315,216,210,143,140,110,94,90]))
